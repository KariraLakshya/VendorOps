...
import { loginRateLimiter, registerRateLimiter } from "../../../middlewares/rateLimit/rateLimiters.js";
import {
  logout,
  register,
  verifyLogin,
 ...
 */
router.post("/register", registerRateLimiter, wrapHandler(register));

/**
 * POST /verify-login - Verify user login credentials
 * Validates user credentials and initiates login process
 */
router.post("/verify-login", loginRateLimiter, wrapHandler(verifyLogin));

/**
 * POST /verify-totp - Verify TOTP for two-factor authentication
 * Validates TOTP code for users with 2FA enabled — also rate-limited,
 * since a 6-digit code is brute-forceable on its own if login itself is
 * already past.
 */
router.post("/verify-totp", loginRateLimiter, wrapHandler(verifyTOTP));
